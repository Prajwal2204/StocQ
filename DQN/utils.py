...
def plot_portfolio_performance_comparison(stock_name, agent):
	...
	if '^GSPC' not in stock_name:
		...
		plt.plot(dates, GSPC_buy_and_hold_portfolio_values, color='red', label='S&P 500 Buy and Hold Total Return: ${:.2f}'.format(GSPC_buy_and_hold_return))
	plt.xticks(np.linspace(0, len(dates), 10))
	plt.ylabel('Portfolio Value ($)')
	plt.legend()
	plt.grid()
	plt.savefig('pics/{}.png'.format(stock_name))
